...
import json
import re
from PyQt5.QtWidgets import QScrollArea,QSizePolicy,QApplication, QMainWindow, QLabel, QVBoxLayout,QHBoxLayout, QPushButton, QWidget, QFileDialog, QListWidget, QStatusBar
...
class ImageLabel(QLabel):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            # self.points_scaled.append((x, y))
            self.update()
            self.updateCoordList()
            print(f"Clicked at coordinates: ({'%.3f' % scaled_x}, {'%.3f' % scaled_y})")
    ...
    def wheelEvent(self, event):
        ...
        if event.modifiers() == Qt.ControlModifier:
            ...
            # 限制缩放因子的范围
            # self.scale_factor = max(0.1, min(self.scale_factor, 2.0))

            # 根据新的缩放因子重新缩放图像
            scaled_image = self.image.scaled(self.pixmap.width() * self.scale_factor,
    ...
    def paintEvent(self, event):
        ...
        for point in self.points:
            painter.drawPoint(point[0]*self.scale_factor, point[1]*self.scale_factor)


    def updateCoordList(self):
        self.coord_list_widget.clear()
        for i, point in enumerate(self.points):
            self.coord_list_widget.addItem(f"Point {i+1}: ({'%.3f' % point[0]}, {'%.3f' % point[1]})") 
    ...
    def loadImage(self):
        ...
        self.setAlignment(Qt.AlignTop | Qt.AlignLeft) # 左边和右边边缘对齐，也就是固定图像左上角坐标
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.coord_list_widget.setFixedWidth(200)
